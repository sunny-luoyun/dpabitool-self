function dFCstd(A_path, prefix, suffix, D_path)
    % 定义用户需要提供的两部分文件夹名称
                ...
                if exist(fullfile(B_folder_path, C_name), 'dir')
                    ...
                    for j = 1:length(files)
                        if ~files(j).isdir % 确保是文件而不是文件夹
                            % 提取文件名中的数字
                            tokens = regexp(files(j).name, ['StdzFC_ROI(\d+)', '_Sub'], 'tokens');
                            if ~isempty(tokens)
                                ROI_number = tokens{1}{1};

                                % 根据ROI数字构建新的文件夹名
                                fc_folder_name = sprintf('%s%s', 'stdFC', ROI_number);
