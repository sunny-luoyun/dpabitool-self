function dFCmean(A_path, prefix, suffix, D_path)
    % 定义用户需要提供的两部分文件夹名称
    ...
    for i = 1:length(foldersA)
        if foldersA(i).isdir && ~strcmp(foldersA(i).name, '.') && ~strcmp(foldersA(i).name, '..')
            % 检查文件夹名称是否以Results结尾
            if contains(foldersA(i).name, 'Results')
                disp(['找到包含Results的文件夹: ', foldersA(i).name]);

                % 构建B文件夹的完整路径
                B_folder_path = fullfile(A_path, foldersA(i).name,'TemporalDynamics/TemporalDynamicsMetrics/');
                ...
                if exist(fullfile(B_folder_path, C_name), 'dir')
                    ...
                    for j = 1:length(files)
                        if ~files(j).isdir % 确保是文件而不是文件夹
                            % 提取文件名中的数字
                            tokens = regexp(files(j).name, ['MeanzFC_ROI(\d+)', '_Sub'], 'tokens');
                            if ~isempty(tokens)
                                ROI_number = tokens{1}{1};

                                % 根据ROI数字构建新的文件夹名
                                fc_folder_name = sprintf('%s%s', 'meanFC', ROI_number);
                                ...
                                % 构建目标文件夹路径
                                dest_folder_path = fullfile(D_path,'Dynamic', fc_folder_name,folderName,  sprintf('S%s', S_number));
