function RFS(A_path, prefix, suffix, D_path)
    % 定义用户需要提供的两部分文件夹名称
    ...
    for i = 1:length(foldersA)
        if foldersA(i).isdir && ~strcmp(foldersA(i).name, '.') && ~strcmp(foldersA(i).name, '..')
            % 检查文件夹名称是否以Results结尾
            if endsWith(foldersA(i).name, 'ResultsS')
                disp(['找到包含ResultsS的文件夹: ', foldersA(i).name]);
                ...
                if exist(fullfile(B_folder_path, C_name), 'dir')
                    ...
                    % 找到所有以z + prefix + Map开头的文件
                    file_pattern = ['sz', prefix, 'Map*'];
                    files = dir(fullfile(C_folder_path, file_pattern));

                    % 重新命名文件夹
                    if strcmp(foldersA(i).name, 'ResultsS')
                        new_folder_name = sprintf('S%d', s_index);
                        s_index = s_index + 1;
                    else
                        % 使用正则表达式提取数字并生成新的文件夹名
                        tokens = regexp(foldersA(i).name, 'S(\d+)_ResultsS', 'tokens');
                        if ~isempty(tokens)
                ...
                disp(['文件夹: ', foldersA(i).name, ' 不以ResultsS结尾']);
            end
        end
    end
